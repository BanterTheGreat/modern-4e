...
#import "../helpers/configuration.typ" as config;

#let themesPage() = {
  let defaultThemePage() = {
    return [
  ...
  let zeitgeistThemePage() = {
    return [
      For the Themes in the Zeitgeist campaign, please refer to the Zeitgeist player guide.
    ]
  }

  return [
    #theme.pageTitle(title: "Themes");
    #if (config.currentBuild == config.zeitgeistBuild) {
      zeitgeistThemePage()
    } else {
      defaultThemePage()
    }
